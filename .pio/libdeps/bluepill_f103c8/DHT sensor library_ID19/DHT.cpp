...
// ESP8266 right now, perhaps bugs in direct port access functions?).
#else
  // uint32_t before_time = micros(); // TODO Workwround to fix the procesor hanging in the loop.
  ...
  //   count++;
  //   end = micros();
  //   if (end - start >= _maxcycles || start > end) return TIMEOUT; // Exceeded timeout, fail.
  // }
  // //   while (digitalRead(_pin) == level) {
  //     return TIMEOUT; // Exceeded timeout, fail.
  // }
